...
public class GenerationBiome implements GenerationCarte{

	protected int[] tailleCarte;
	protected double niveauEau;
	
	private long seedElevation;
	private long seedHumidite;
	...
	public GenerationBiome(int[] tailleCarte, double niveauEau) {
	...
	public Carte createCarteOrigine(Bruit bruit, Filtre filtre){
		...
		this.seedHumidite = this.seedElevation + 100;
		...
		for(int x =0 ; x<this.tailleCarte[0];x++) {
			for(int y =0 ; y<this.tailleCarte[1];y++) {
				...
				this.bruit.setSeed(this.seedHumidite);
				double humidite = this.bruit.getNoise(x, y);
				carte[x][y] = remplissageCarte(elevation, humidite);
			}
		}
		...
		if(choixUtilisateurX != 0) {
			...
			for(int x = valeurXgeneration; x < superieurXgeneration; x++) {
				for(int y = 0; y < this.tailleCarte[1]; y++) {
					...
					this.bruit.setSeed(this.seedHumidite);
					double humidite = this.bruit.getNoise(xCam + x, this.sauvegardeCamY + y);
					nouvelleCarte[x][y] = remplissageCarte(elevation, humidite);
				}
			}
		...
		if(choixUtilisateurY != 0) {
			...
			for(int x = 0; x < this.tailleCarte[0]; x++) {
				for(int y = valeurYgeneration; y < superieurYgeneration; y++) {
					...
					this.bruit.setSeed(this.seedHumidite);
					double humidite = this.bruit.getNoise(xCam + x, yCam + y);
					nouvelleCarteDiagonale[x][y] = remplissageCarte(elevation, humidite);
				}
			}
	...
	public Point remplissageCarte(double elevation, double humidite) {
		Point newPoint;
		...
		int newHumidite = (int) (humidite*100);
		double elevationClone = elevation;
		if(elevation <= this.niveauEau) {
			elevation = this.niveauEau;
		}
		
		//Cela nous evite d'avoir des lacs trop fonce, tout en gardant un degrade
		if(elevationClone <= 0.5) {
			int rouge = (int)(this.tableauCouleurCarte[newElevation][newHumidite].getRed()*(0.85-elevationClone));
			int vert = (int)(this.tableauCouleurCarte[newElevation][newHumidite].getGreen()*(0.85-elevationClone));
			int bleu = (int)(this.tableauCouleurCarte[newElevation][newHumidite].getBlue()*(0.85-elevationClone));
			newPoint = new Point((new Color(rouge, vert, bleu)), elevation, humidite);
		}
		else {
			int rouge = (int)(this.tableauCouleurCarte[newElevation][newHumidite].getRed()*elevation);
			int vert = (int)(this.tableauCouleurCarte[newElevation][newHumidite].getGreen()*elevation);
			int bleu = (int)(this.tableauCouleurCarte[newElevation][newHumidite].getBlue()*elevation);
			newPoint = new Point((new Color(rouge, vert, bleu)), elevation, humidite);
		}
		return newPoint;
	}
